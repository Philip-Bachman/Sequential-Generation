...
def test_imocld_imp_mnist(step_type='add', occ_dim=14, drop_prob=0.0, attention=False):
    ...
    out_file = open("TBCLM_IMP_MNIST_RESULTS_OD{}_DP{}_{}_{}.txt".format(occ_dim, dp_int, step_type, att_tag), 'wb')
    out_file.flush()
    ...
    for i in range(250000):
        ...
        if ((i % 1000) == 0):
            ...
            for j in xrange(n_iter):
                ...
                img.save("TBCLM-IMP-MNIST-OD{0:d}-DP{1:d}-{2:s}-samples-{3:03d}.png".format(occ_dim, dp_int, step_type, j))
...
def test_imocld_imp_tfd(step_type='add', occ_dim=14, drop_prob=0.0, attention=False):
    ...
    for i in [0]: #range(250000):
        ...
        if ((i % 1000) == 0):
            ...
            for j in xrange(n_iter):
                img = img_grid(samples[j,:,:,:])
                img.save("TBCLM-IMP-TFD-OD{0:d}-DP{1:d}-{2:s}-samples-{3:03d}.png".format(occ_dim, dp_int, step_type, j))
