...
class SeqCondGen2dS(BaseRecurrent, Initializable, Random):
    ...
    ***                                                                     ***
    ...
    def apply(self, x, y, u, u_att, nll_scale, c, h_con, c_con, h_rav, c_rav):
        ...
        i_rav = self.rav_mlp_in.apply(tensor.concatenate( \
                        [nll_grad, z_slf, read_out, att_specs], axis=1))
        h_rav, c_rav = self.rav_rnn.apply(states=h_rav, cells=c_rav, \
                                          inputs=i_rav, iterate=False)
...
class SeqCondGenX(BaseRecurrent, Initializable, Random):
    ...
    def apply(self, x, y, u, u_att, nll_scale, c, h_con, c_con, h_gen, c_gen, h_var, c_var):
        ...
        # condition on NLL gradient information
        nll_grad = y - c_as_y

        # update guide controller
        #i_rav = self.rav_mlp_in.apply( \
        #        tensor.concatenate([nll_grad, h_con, h_obs], axis=1))
        #h_rav, c_rav = self.rav_rnn.apply(states=h_rav, cells=c_rav,
        #                                  inputs=i_rav, iterate=False)
        #q_as_mean, q_as_logvar, q_att_spec = \
        ...
        grad_out = self.reader_mlp.apply(nll_grad, nll_grad, att_spec)
        ...
        i_gen = self.gen_mlp_in.apply( \
                tensor.concatenate([read_out, grad_out, att_spec], axis=1))
        h_gen, c_gen = self.gen_rnn.apply(states=h_gen, cells=c_gen,
                                          inputs=i_gen, iterate=False)
        # update the guide RNN state
        i_var = self.var_mlp_in.apply( \
                tensor.concatenate([nll_grad, read_out, grad_out, att_spec], axis=1))
        h_var, c_var = self.var_rnn.apply(states=h_var, cells=c_var,
                                          inputs=i_var, iterate=False)
...
############################################################
############################################################
## ATTENTION-BASED IMPUTATION UNDER TIME CONSTRAINTS      ##
############################################################
############################################################

class SeqCondGenIMP(BaseRecurrent, Initializable, Random):
    """
    SeqCondGenIMP -- constructs conditional densities under time constraints.
    ...
    def __init__(self, x_and_y_are_seqs,
        ...
        self.x_dim = x_dim
        self.y_dim = y_dim
        assert (not x_and_y_are_seqs), "only static inputs allowed!"
        assert (x_dim == y_dim), "x and y will have the same dimension!"
        # construct a sequence of scales for measuring NLL. we'll use scales
        # corresponding to some fixed number of guaranteed steps, followed by
        # a constant probability of early stopping. any "residual" probability
        # will be picked up by the final step.
    ...
    def get_dim(self, name):
        if name == 'x':
            ...
            super(SeqCondGenIMP, self).get_dim(name)
        return

    #------------------------------------------------------------------------

    @recurrent(sequences=['x', 'm', 'u', 'u_att', 'nll_scale'], contexts=[],
               states=['c', 'h_con', 'c_con', 'h_gen', 'c_gen', 'h_var', 'c_var'],
               outputs=['c', 'h_con', 'c_con', 'h_gen', 'c_gen', 'h_var', 'c_var', 'c_as_y', 'nll', 'kl_q2p', 'kl_p2q', 'kl_amu', 'kl_alv', 'att_map', 'read_img'])
    def apply(self, x, m, u, u_att, nll_scale, c, h_con, c_con, h_gen, c_gen, h_var, c_var):
        if self.step_type == 'add':
            # additive steps use c as a "direct workspace", which means it's
            # already directly comparable to y.
            c_as_y = tensor.nnet.sigmoid(c)
        ...
        x_m = (m * x) + ((1. - m) * c_as_y)
        # make an input of diff between input and predictions
        x_d = x - c_as_y
        ...
        # apply the attention-based reader to the input in x
        read_out = self.reader_mlp.apply(x_m, x_m, att_spec)
        diff_out = self.reader_mlp.apply(x_d, x_d, att_spec)
        att_map = self.reader_mlp.att_map(att_spec)
        read_img = self.reader_mlp.write(read_out, att_spec)
    ...
    def process_inputs(self, x, m):
        ...
        if self.x_and_y_are_seqs:
            ...
            batch_size = x.shape[0]
            x = x.dimshuffle('x',0,1).repeat(self.total_steps, axis=0)
            m = m.dimshuffle('x',0,1).repeat(self.total_steps, axis=0)

        # get initial states for all model components
        c0 = self.c_0.repeat(batch_size, axis=0)
        cc0 = self.cc_0.repeat(batch_size, axis=0)
        u_hc0 = 0.02 * self.theano_rng.normal( \
        ...
        # run the multi-stage guided generative process
        cs, _, _, _, _, _, _, c_as_ys, nlls, kl_q2ps, kl_p2qs, kl_amus, kl_alvs, att_maps, read_imgs = \
                self.apply(x=x, m=m, u=u, u_att=u_att,
                             nll_scale=self.nll_scales,
                             c=c0,
                             h_con=hc0, c_con=cc0,
                             h_gen=hg0, c_gen=cg0,
    ...
    def build_attention_funcs(self):
        ...
        if self.x_and_y_are_seqs:
            x_sym = tensor.tensor3('x_sym_att_funcs')
            m_sym = tensor.tensor3('m_sym_att_funcs')
        else:
            x_sym = tensor.matrix('x_sym_att_funcs')
            m_sym = tensor.matrix('m_sym_att_funcs')
        # collect estimates of y given x produced by this model
        xs, cs, c_as_ys, nlls, kl_q2ps, kl_p2qs, kl_amus, kl_alvs, att_maps, read_imgs = \
                self.process_inputs(x_sym, m_sym)
        ...
        sample_func = theano.function(inputs=inputs, \
                                      outputs=outputs)
        def switchy_sampler(x, m, sample_source='q'):
            # store value of sample source switch, to restore later
            old_switch = self.train_switch.get_value()
            if sample_source == 'p':
                # take samples from the primary policy
...
class SeqCondGenRAM(BaseRecurrent, Initializable, Random):
    """
    ...
    def get_dim(self, name):
        if name == 'x':
            ...
            super(SeqCondGenRAM, self).get_dim(name)
        return
