...
class SeqCondGenOBS(BaseRecurrent, Initializable, Random):
    """
    ...
    def __init__(self, x_and_y_are_seqs,
                    ...
                    att_noise=0.05,
                    **kwargs):
        super(SeqCondGenOBS, self).__init__(**kwargs)
        # record basic structural parameters
        self.x_and_y_are_seqs = x_and_y_are_seqs
        self.total_steps = total_steps
        self.init_steps = init_steps
    ...
    def _allocate(self):
        ...
        # add the theano shared variables to our parameter lists
        self.params.extend([ self.c_0, \
                             self.cc_0, self.co_0, self.cv_0, \
                             self.hc_0, self.ho_0, self.hv_0 ])
        return
    ...
    def get_dim(self, name):
        if name == 'x':
            ...
            super(SeqCondGenOBS, self).get_dim(name)
        return

    #------------------------------------------------------------------------

    @recurrent(sequences=['x', 'y', 'u', 'u_att', 'nll_scale'], contexts=[],
               states=['c', 'h_con', 'c_con', 'h_obs', 'c_obs', 'h_var', 'c_var'],
               outputs=['c', 'h_con', 'c_con', 'h_obs', 'c_obs', 'h_var', 'c_var', 'c_as_y', 'nll', 'kl_q2p', 'kl_p2q', 'kl_amu', 'kl_alv', 'att_map', 'read_img'])
    def apply(self, x, y, u, u_att, nll_scale, c, h_con, c_con, h_obs, c_obs, h_var, c_var):
        ...
        kl_alv = tensor.sum(gaussian_logvar_kld(as_mean, as_logvar, \
                            0., 0.), axis=1)
        return c, h_con, c_con, h_obs, c_obs, h_var, c_var, c_as_y, nll, kl_q2p, kl_p2q, kl_amu, kl_alv, att_map, read_img
...
class SeqCondGenALL(BaseRecurrent, Initializable, Random):
    ...
    def apply(self, x, y, u, u_att, nll_scale, c, h_con, c_con, h_obs, c_obs, h_var, c_var, h_rav, c_rav):
        # non-additive steps use c_con as a "latent workspace", which means
        # it needs to be transformed before being comparable to y.
        c_as_y = tensor.nnet.sigmoid(self.writer_mlp.apply(h_con))
        # compute difference between current prediction and target value
